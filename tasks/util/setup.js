...
module.exports = function(answers) {
  ...
  var old = (config.after_success || []).indexOf('grunt ghrelease');
  if (config.after_success && old !== -1) {
    console.log('Removed deprecated `grunt ghrelease` from `.travis.yml`.');
    delete config.after_success[old];
    // sparse array fun
    if (!config.after_success.join('').split('').length) {
      delete config.after_success;
    }
  }
